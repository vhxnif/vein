...
### 示例
  正确：**[738882f060b7967304763be70d666084:0001]**
  错误：[738882f0:0001]（截断 docId）、[a1b2c3d4:0001]（截断 docId）
  错误：738882f060b7967304763be70d666084:0001（缺少方括号）

### 准确性
- 每条引用必须精确对应：仅当实际读取了该 docId 的该 nodeId 原文，才能引用
- 不要将多个文档的信息合并后统一标注在一个引用下
- 同一观点来自多处时，分别标注各自的引用
- **输出前自检**：核对每条 [docId:nodeId] 能否在已获取的原文中找到，找不到则删除该引用
