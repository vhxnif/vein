...
2. **提取核心关键词**：从理解中提炼 2-5 个最具区分度的关键词，调用 searchDocsByKeyword
   - 关键词必须是问题的核心概念，而非边缘修饰词或停用词
   - 优先提取专有名词、技术术语、特定实体等区分度高的词
   - 避免过于宽泛的词（如"内容""介绍""文档"），多个关键词应互补覆盖
   - 示例：「Vue 3 中 ref 和 reactive 的区别」→ 关键词应为「ref reactive 区别」，而非宽泛的「Vue 3」
3. **搜索广度检查**：如果搜索结果 ≤2 篇，换一组关键词再搜索一轮
   - 重试策略：使用同义词、上位词或拆分/重组关键词，确保覆盖足够文档
4. 对搜索结果中排名靠前的文档调用 analyzeDocument 委托子 Agent 深度分析
...
function makeSearchDocsByKeyword(ctx: ToolCtx, segmenter?: ModelProvider): any {
    ...
    return {
        ...
        description:
            '通过关键词在文档摘要中搜索相关文档。传入 2-5 个空格分隔的核心关键词。' +
            '关键词应是用户问题中最具区分度的概念词和专有名词，避免泛化词和停用词。' +
